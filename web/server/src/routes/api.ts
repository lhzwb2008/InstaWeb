...
import { parseFileStructure } from '../../../../src/utils/file.js';
import { getOutputDirectory, getPreviewHost, getPreviewPort, getConfig } from '../../../../src/utils/config.js';
import { createServer } from '../../../../src/utils/file.js';
import { sessionMap, sessionOutputDirs } from '../index.js';
...
function createUniqueOutputDir(sessionId: string) {
  ...
  const outputDir = path.join(absoluteBaseDir, folderName);
...
function findMostRecentWebappDir(sessionId: string): string | null {
  try {
...
export function createApiRouter(io: SocketIOServer) {
  ...
  router.post('/create', async (req, res) => {
    try {
      ...
      apiHandler.on = (event: string, listener: any) => {
        if (event === 'data') {
          planEmitter.emit = (emitEvent: string, data: any) => {
            if (emitEvent === 'data') {
              listener(data);
              io.to(sessionId).emit('generation-progress', { type: 'plan-data', data });
            }
      ...
      // Get the socket ID from the request to target the specific client
      const socketId = req.headers['x-socket-id'] as string;
      
      // If we have a socket ID, emit to that specific socket
      if (socketId) {
        ...
        if (socket) {
          ...
          // Fallback to room if socket not found directly
          io.to(sessionId).emit('generation-progress', { 
            type: 'generation-complete', 
            data: { 
              files: generatedFiles,
  ...
  router.get('/preview', (req, res) => {
    try {
      ...
      const previewDir = path.join(__dirname, '../../../../web/client/preview');
...
`;
      ...
      const updatedOutputDir = path.join(absoluteBaseDir, updatedFolderName);
  ...
  router.get('/demo', (req, res) => {
    const demoPath = path.join(__dirname, '../../../../web/client/demo.html');
